...
import { ArtistRevenue, DetailsHeader, Discography, Loader } from "../../components";
...
const ArtistAccount = () => {
  ...
  const {data, isLoading, isFetching, isError, isSucces} = useGetMeQuery(user.id)
  const navigate = useNavigate()
  
  if (isFetching) return <Loader title="Artist Page...." />;
  if (isFetching) return <Loader title="Artist songs...." />;
  
  return (
    <>
      {data?.role.name === 'Artist' ?
      ...
      <div className="flex md:flex-row sm:flex-col sm:w-full w-full flex-wrap gap-5">
        <div className="lg:w-1/4 md:w-1/4 sm:w-full">
          <div className="flex md:w-full sm:w-full flex-col text-base font-semibold flex-wrap gap-3  m-2 p-2 mb-20 rounded-2xl shadow-md shadow-orange-600 h-auto">
            <span
              ...
              className="flex cursor-pointer text-white flex-wrap gap-3 p-2 m-2 justify-start items-center"
            >
              {" "}
        ...
        {selected === "infos" && (
          <div className="md:w-3/5 lg:w-3/5 sm:w-full  rounded-2xl flex flex-col p-2 m-2 shadow-md shadow-orange-600">
            <div className="w-full mb-10 flex justify-center items-center flex-col flex-wrap gap-4">
              <h2 className="text-slate-200 text-xl font-semibold   rounded-2xl">
                Details personnels
              </h2>
              ...
              <div className="w-full">
                <div className="flex justify-around ">
                  <Input
                    className="w-1/2"
                    default={artistData?.data?.attributes.name}
                    value={artistData?.data?.attributes.name}
                    name={"Nom"}
                    useRef={ref}
                    element="input"
                  ...
                  <Input
                    className="w-1/2"
                    value={artistData?.data?.attributes.genre}
                    name={"Genre"}
                    useRef={ref}
                    element="input"
                ...
                <div className="flex justify-around">
                  <Input
                    className="w-1/2"
                    value={artistData?.data.attributes.date}
                  ...
                  <Input
                    className="w-1/2"
                    default={artistData?.data.attributes.email}
                    name={"email"}
                    useRef={ref}
                    element="input"
                ...
                <Input
                  name={"Téléphone"}
                  value={artistData?.data.attributes.phone}
                  useRef={ref}
                  element="input"
              ...
              <div className="w-full">
                <div className="flex justify-around ">
                  <Input
                    ...
                    value={artistData?.data.attributes.Pays}
                    name={"Pays"}
                    useRef={ref}
                    element="input"
                    onInput={(e) => console.log(e)}
                  ...
                  <Input
                    ...
                    value={artistData?.data.attributes.ville}
                    name={"Ville"}
                    useRef={ref}
                    element="input"
                    onInput={(e) => console.log(e)}
                ...
                <div className="flex justify-around">
                  <Input
                    className="w-1/2 capitalize"
                    value={artistData?.data.attributes.adresse}
                    name={"Adresse"}
                    useRef={ref}
                    element="input"
                    onInput={(e) => console.log(e)}
        ...
        {selected === "revenus" && <ArtistRevenue artistID={artistId} />}

        {selected === "disco" && <Discography artistID={artistId} />}
      </div>
    </div> : 

    (toast.error('Vous devez être artiste pour accéder à cette page'), navigate('/'))
      }
    </>
